...
this.drawEndHandler=function(b){if(!a.settings.readOnly){try{b.preventDefault()}catch(F){}d.clear();a.dataEngine.endStroke()}};
// 开始画图
this.drawStartHandler=function(h){
  if(!a.settings.readOnly){
    ...
    d.kick();
    if ($(a.canvas).closest(".signature_wrap") && $(a.canvas).closest(".signature_wrap").find(".sign_placeholder")) {
      $(a.canvas).closest(".signature_wrap").find(".sign_placeholder").hide();
    }
  }
};
this.drawMoveHandler=function(b){a.settings.readOnly||(b.preventDefault(),a.dataEngine.inStroke&&(a.dataEngine.addToStroke(e(b)),d.kick()))};return this}.call({},this);(function(a,b,c){var e=this.canvas,h=
...
// 判断是否有placeholder,有值时隐藏，无时显示
if (a.closest(".signature_wrap") && a.closest(".signature_wrap").find(".sign_placeholder")) {
  if (a.jSignature("getData", "native").length > 0) {
    a.closest(".signature_wrap").find(".sign_placeholder").hide();
  } else {
    a.closest(".signature_wrap").find(".sign_placeholder").show();
  }
}
a.trigger("change")}}(this.$parent,
...
d.settings[a]=b;d.resetCanvas(c?null:d.settings.data,!0);return d.settings[a]},clear:c,reset:c,addPlugin:function(a,b,c){p.hasOwnProperty(a)&&(p[a][b]=c);return this},listPlugins:function(a){var b=[];if(p.hasOwnProperty(a)){a=p[a];for(var c in a)a.hasOwnProperty(c)&&b.push(c)}return b},getData:function(a){var b=this.find("canvas.jSignature").add(this.filter("canvas.jSignature"));void 0===a&&(a="default");if(0!==b.length&&g.hasOwnProperty(a))return g[a].call(b.get(0),b.data("jSignature.data"),b.data("jSignature.settings"))}
		,'setColor' : function(color) {
		     // 获取 context 对象
		     var context2D = this.find('canvas.jSignature')
		                     .add(this.filter('canvas.jSignature'))
		                     .data('jSignature.this').canvasContext
		     //设置阴影的颜色
